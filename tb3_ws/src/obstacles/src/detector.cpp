...
int main(int argc, char * argv[])
{
  ...
  double obs_angle_min = node->get_parameter("obs_angle_min").get_parameter_value().get<double>();
  double obs_angle_max = node->get_parameter("obs_angle_max").get_parameter_value().get<double>();
  double obs_threshold = node->get_parameter("obs_threshold").get_parameter_value().get<double>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
